...
Festiverse Backend — FastAPI 기반 페스티벌/공연 정보 큐레이션 서비스

## Commands

- **Run dev server**: `uvicorn main:app --reload --host 0.0.0.0 --port 8000`
- **Run directly**: `python main.py` (starts on port 8000)
...
- ORM, External API, Environment 설정 등의 올바른 위치 규정
...
- KOPIS API (공연예술통합전산망) — 공연/페스티벌 목록 및 상세 정보
- 네이버 블로그 검색 API — 공연별 관련 블로그 수집
...
- Database / External API는 Infrastructure 또는 Adapter에서만 사용해야 한다.
